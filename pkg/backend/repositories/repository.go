...
func (r *Repository) SaveToPg(document *schemas.DocumentMetadata, pages *[]schemas.PageMetadata) error {
	err := r.db.Transaction(func(tx *gorm.DB) error {
		err := tx.Table("document_metadata").Save(&document).Error
		if err != nil {
			return errors.Wrap(err, "failed to save document metadata")
		}

		err = tx.Table("page_metadata").Save(&pages).Error
		if err != nil {
			return errors.Wrap(err, "failed to save pages")
		}

		//err = tx.Table("attributes").Save(&).Error
		//if err != nil {
		//	return errors.Wrap(err, "failed to save attributes")
		//}
		return nil
	})
	if err != nil {
